document.addEventListener('DOMContentLoaded', function() {
  ...
  const searchContainer = document.querySelector('.search-container-pg-adm');
  const menuPagadm = document.querySelector('.menu-pg-adm');
  
  // Create mobile menu container
  const mobileMenuContainer = document.createElement('div');
  mobileMenuContainer.className = 'mobile-menu-container';
  menuPagadm.appendChild(mobileMenuContainer);
  
  menuToggle.addEventListener('click', function() {
    const isMobileView = window.matchMedia('(max-width: 360px)').matches;
    ...
    if (mobileMenuContainer.classList.contains('active')) {
      ...
      document.querySelector('.menu-content-pg-adm').insertBefore(navLinks, menuToggle);
      document.querySelector('.menu-content-pg-adm').insertBefore(searchContainer, menuToggle);
  ...
  document.querySelectorAll('.nav-item-pg-adm, .search-container-pg-adm button').forEach(element => {
  ...
  document.querySelectorAll('.nav-item-pg-adm').forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      const isCorrectPage = currentPath.endsWith('/pg_adm_cliente.php') || currentPath.endsWith('/pg_adm_cliente');
      ...
      if (isCorrectPage && targetElement) {
        ...
        console.log('Redirecting to pg_adm_cliente.php with hash');
        window.location.href = 'pg_adm_cliente.php' + targetId;
      }
    });
  });
...
document.addEventListener('click', function(event) {
  if (!menuPagadm.contains(event.target)) {
    closeMobileMenu();
  }
});
...
function closeMobileMenu() {
  ...
  const menuContent = document.querySelector('.menu-content-pg-adm');
